  ...
  useEffect(() => {
    ...
    // For single selection
      ...
      if (defaultValue !== currentValue) {
        ...
        // If not found by name, try to find by alpha3 code
        if (!initialCountry) {
        ...
        if (
          initialCountry &&
          selectedCountries[0]?.alpha3 !== initialCountry.alpha3
        ) {
  ...
  const handleSelect = useCallback(
    (country: Country) => {
      if (multiple) {
        ...
        // Skip update if the same country is already selected
        if (
          selectedCountries.length === 1 &&
          selectedCountries[0].alpha3 === country.alpha3
        ) {
          setOpen(false);
          return;
        }
